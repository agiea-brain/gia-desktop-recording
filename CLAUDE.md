...
Gia is an Electron desktop app (macOS + Windows) for meeting recording using the Recall.ai Desktop SDK. It detects active meetings, records with user confirmation, provides live transcription, and generates AI-powered summaries.

## Commands

```bash
npm start                # Run dev app (electron-forge start)
...
- **Renderer processes** — Meeting popup, onboarding, debug controls (each with own preload + HTML)
- **Preload bridges** (`src/*-preload.js`) — Expose `window.electronAPI` via `contextBridge` per window
...
2. Add wrapper in the relevant `*-preload.js` under `electronAPI`
3. Call from renderer via `window.electronAPI.action()`

### Two Critical Backend Interactions

1. **Upload token** — On recording start, the app fetches an upload token from the Gia backend (`api.getUploadToken`). This token is passed to `RecallAiSdk.startRecording()`.
...
- `src/utils/logger.js` — Console + remote Logfire logging. W3C trace context support
- `src/utils/load-env.js` — Smart `.env` discovery across dev/packaged contexts

### Recall SDK Integration

Recording flow: meeting-detected event → popup for user confirmation → auth check → upload token from backend → `startRecording(windowId, uploadToken)` → realtime transcript/video events → recording-ended → state reset.
