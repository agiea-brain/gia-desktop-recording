...
const WINDOWS_REQUIRED_SIGN_ENV_VARS = [
	'SIGNTOOL_PATH',
	'AZURE_CODE_SIGNING_DLIB',
	'AZURE_METADATA_JSON',
];
const WINDOWS_PUBLISHER = process.env.GIA_WINDOWS_PUBLISHER || 'Agiea Inc.';
const WINDOWS_APP_DESCRIPTION = process.env.GIA_WINDOWS_APP_DESCRIPTION || 'Gia - AI Meeting Assistant';

if (SHOULD_SIGN_WINDOWS) {
	const missingWindowsSignEnvVars = WINDOWS_REQUIRED_SIGN_ENV_VARS.filter(
		(key) => !process.env[key] || process.env[key].trim() === '',
	);
	if (missingWindowsSignEnvVars.length > 0) {
		throw new Error(
			`Windows signing enabled but missing env vars: ${missingWindowsSignEnvVars.join(', ')}`,
		);
	}
}
const WINDOWS_SIGN_OPTIONS = SHOULD_SIGN_WINDOWS
	? {
		signToolPath: process.env.SIGNTOOL_PATH,
		hashes: ['sha256'],
		signWithParams:
			'/dlib ' + process.env.AZURE_CODE_SIGNING_DLIB + ' /dmdf ' + process.env.AZURE_METADATA_JSON,
	}
	: undefined;
...
module.exports = {
	packagerConfig: {
		...
		windowsSign: WINDOWS_SIGN_OPTIONS,
		win32metadata: {
			CompanyName: WINDOWS_PUBLISHER,
			ProductName: 'Gia',
			FileDescription: WINDOWS_APP_DESCRIPTION,
		},
