...
function postToLoggingApi(payload, { traceparentOverride = null, startTrace = false } = {}) {
...
function emit(level, args, context) {
    ...
    const traceparentOverrideRaw = mergedContext.traceparent || mergedContext.traceParent || null;
    ...
    if (teeToConsole) {
        try {
            ...
            if (fn) {
                ...
                if (Object.keys(mergedContext).length) fn(ts, ...args, mergedContext);
                else fn(ts, ...args);
            }
        } catch {
    ...
    if (traceEnabled && !currentTrace?.traceparent) {
        ...
        traceStartInFlight = Promise.resolve(postToLoggingApi(remotePayload, { startTrace: true }))
            .then(async (res) => {
...
const logger = {
    configure(options = {}) {
        ...
        loggingApiUrl = typeof apiUrl === 'string' && apiUrl.length ? apiUrl : null;
